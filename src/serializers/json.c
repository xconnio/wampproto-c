...
static cJSON *value_to_cjson(const Value *val) {
    ...
    switch (val->type) {
        case VALUE_NULL: return cJSON_CreateNull();
        case VALUE_BOOL: return cJSON_CreateBool(val->bool_val);
        case VALUE_INT:  return cJSON_CreateNumber((double)val->int_val);
        case VALUE_FLOAT:return cJSON_CreateNumber(val->float_val);
        case VALUE_STR:  return cJSON_CreateString(val->str_val);
        case VALUE_BYTES: {
