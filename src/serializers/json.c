...
static cJSON* value_to_cjson(const Value* val) {
    ...
    switch (val->type) {
        ...
        case VALUE_BYTES: {
            ...
            for (size_t i = 0; i < val->bytes_val.len; i++) sprintf(&hex[i * 2], "%02x", val->bytes_val.data[i]);
...
static Value* cjson_to_value(const cJSON* json) {
    ...
    if (cJSON_IsNull(json)) return value_null();
    if (cJSON_IsBool(json)) return value_bool(cJSON_IsTrue(json));
    ...
    if (cJSON_IsString(json)) return value_str(json->valuestring);
    if (cJSON_IsArray(json)) {
        size_t size = cJSON_GetArraySize(json);
        Value* arr = value_list(0);
        cJSON* child;
        cJSON_ArrayForEach(child, json) { value_list_append(arr, cjson_to_value(child)); }
        return arr;
    }
    if (cJSON_IsObject(json)) {
        Value* dict = value_dict();
        cJSON* child;
        cJSON_ArrayForEach(child, json) { value_dict_set(dict, child->string, cjson_to_value(child)); }
        return dict;
    }
...
static void json_free(Serializer* self) { free(self); }
