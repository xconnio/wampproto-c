...
static List* publish_marshal(const Message* self) {
    ...
    int length = 4;
    if (publish->args != NULL) length = 5;
    if (publish->kwargs != NULL) length = 6;
    ...
    if (publish->args != NULL) value_list_append(list, value_from_list(publish->args));

    if (publish->kwargs != NULL) {
        if (publish->args == NULL) value_list_append(list, value_list(0));
...
static void publish_free(Message* self) {
    ...
    if (publish->args != NULL) value_free(value_from_list(publish->args));

    if (publish->kwargs != NULL) value_free(value_from_dict(publish->kwargs));
