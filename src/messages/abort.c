...
static List* abort_marshal(const Message* self) {
    ...
    int length = 3;
    if (abort->args != NULL) length = 4;
    if (abort->kwargs != NULL) length = 5;
    ...
    if (abort->args != NULL) value_list_append(list, value_from_list(abort->args));

    if (abort->kwargs != NULL) {
        if (abort->args == NULL) value_list_append(list, value_list(0));
...
static void abort_free(Message* self) { free(self); }
