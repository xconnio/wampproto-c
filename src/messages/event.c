...
static List *event_marshal(const Message *self)
{
    ...
    if (length >= 5)
        value_list_append(list, value_from_list(event->args));

    if (length == 6)
        value_list_append(list, value_from_dict(event->kwargs));
...
static void event_free(Message *self)
{
    ...
    if (event->args != NULL)
        value_free(value_from_list(event->args));

    if (event->kwargs != NULL)
        value_free(value_from_dict(event->kwargs));
