...
static List* yield_marshal(const Message* self) {
    ...
    int length = 3;
    if (yield->args != NULL) length = 4;
    if (yield->kwargs != NULL) length = 5;
    ...
    if (yield->args != NULL) value_list_append(list, value_from_list(yield->args));

    if (yield->kwargs != NULL) {
        if (yield->args == NULL) value_list_append(list, value_list(0));
...
static void yield_free(Message* self) {
    ...
    if (yield->args != NULL) value_free(value_from_list(yield->args));
    if (yield->kwargs != NULL) value_free(value_from_dict(yield->kwargs));
