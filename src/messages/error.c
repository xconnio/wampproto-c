...
static List* error_marshal(const Message* self) {
    ...
    int length = 5;
    if (error->args != NULL) length = 6;
    if (error->kwargs != NULL) length = 7;
    ...
    if (error->args != NULL) value_list_append(list, value_from_list(error->args));

    if (error->kwargs != NULL) {
        if (error->args == NULL) value_list_append(list, value_list(0));
