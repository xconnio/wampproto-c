...
static List* invocation_marshal(const Message* self) {
    ...
    int length = 4;
    if (invocation->args != NULL) length = 5;

    if (invocation->kwargs != NULL) length = 6;
    ...
    if (invocation->args != NULL) value_list_append(list, value_from_list(invocation->args));

    if (invocation->kwargs != NULL) {
        if (invocation->args == NULL) value_list_append(list, value_list(0));
