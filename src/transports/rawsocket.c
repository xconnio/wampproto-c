...
void handshake_free(Handshake* hs) { free(hs); }

int send_handshake(const Handshake* hs, uint8_t out[4]) {
    if (hs->max_message_size > PROTOCOL_MAX_MSG_SIZE) return -1;

    const double log2_val = log2(hs->max_message_size);
    const int log2_int = (int)log2_val;
    if (1 << log2_int != hs->max_message_size || log2_int < 9) return -2;
...
void message_header_free(MessageHeader* header) { free(header); }
...
int bytes_to_int(const uint8_t b[3]) { return b[0] << 16 | b[1] << 8 | b[2]; }
