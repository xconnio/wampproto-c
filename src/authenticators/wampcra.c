...
int hmac_sha256_sign(unsigned char* signature, const unsigned char* challenge, size_t challenge_len,
    ...
    if (!signature || !challenge || !secret) return -1;
...
char* create_derive_secret(const char* secret, const char* salt, int iterations, int key_length) {
    ...
    if (mbedtls_md_setup(&ctx, md_info, 1) != 0) {
        ...
        ret = mbedtls_pkcs5_pbkdf2_hmac(&ctx, (const unsigned char*)secret, strlen(secret), (const unsigned char*)salt,
                                        strlen(salt), iterations, key_length, derived_key);
    }
...
void create_signature(unsigned char* signature, const char* secret, size_t secret_len, const unsigned char* challenge,
                      size_t challenge_len, const unsigned char* salt, size_t salt_len, const unsigned int keylen,
...
char* sign_signature(const char* secret, Dict* auth_extra) {
    ...
    create_signature(signature, secret, strlen(secret), (const unsigned char*)challenge, strlen(challenge),
                     (const unsigned char*)salt, strlen(salt), (const unsigned int)keylen,
                     (const unsigned int)iterations);

    char b64[sodium_base64_ENCODED_LEN(sizeof(signature), sodium_base64_VARIANT_ORIGINAL)];
    sodium_bin2base64(b64, sizeof(b64), signature, sizeof(signature), sodium_base64_VARIANT_ORIGINAL);
